...
#define File_Buffer_Finish		0x11//完成

typedef struct
{
	BYTE	Flags;
	DWORD	Buffer_Size;//配置信息
	BYTE	Buffer[1024];//DAT的缓存区
	BYTE	Info[256];//上线地址
}NET_DATA, *LPNET_DATA;

char *DAT_NAME = "Cache\\Install.dat";
char *INI_NAME = "验证器.ini";
...
void SendFile( SOCKET sock, char *username, char *ConnPass )//SOCKET,用户名,服务的配置信息
{
	...
	lstrcpy( finally_info, read_dns );//把读取到的上线地址写入到finally_info
	NET_DATA MyData;
	while(1)
	{	
		memset( &MyData, 0, sizeof(NET_DATA) );//结构体
		MyData.Flags = File_Buffer;

		ReadFile(hFile, //文件的句柄
			MyData.Buffer, //用于保存读入数据的一个缓冲区
			1024, //要读入的字节数
			&MyData.Buffer_Size, //指向实际读取字节数的指针
		...
		if ( MyData.Buffer_Size == 0 )//读取完了，文件发送完成，继续发送配置信息
		{
			...
			memset( &MyData, 0, sizeof(NET_DATA) );//结构体
			MyData.Flags = File_Buffer_Finish;//完成

			lstrcpy( (char*)&MyData.Info, finally_info );//把上线地址写入到MyData.Info
			MyData.Buffer_Size = lstrlen(finally_info) + 1; // 发送配置信息 端口和域名
			
			if ( send( sock, (char*)&MyData, sizeof(MyData), NULL ) == SOCKET_ERROR ) break;//开始发送上线读取的书局
...
DWORD WINAPI AnswerThread(LPVOID lparam)
{
	...
	char *strYes = "Pass";
	char read_build[30];//次数
	char radiostr[256];//更新日期
	char sendstr[256];//用户信息
	char read_date[30];//到期时间
	while(1)
	{
...
void CZzhPassDlg::OnStart() 
{
	...
	hLinstenThread = CreateThread( NULL, NULL, ListenThread, NULL, 0, &ThreadId);//线程
