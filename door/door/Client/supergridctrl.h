...
public:
	DWORD	m_dwItemDate;
	void MySetItemDate(DWORD dwDate){m_dwItemDate = dwDate;}
	DWORD MyGetItemDate(){return m_dwItemDate;}
...
public:
	CSuperGridCtrl();
	DECLARE_DYNCREATE(CSuperGridCtrl)
protected:
	//nested class forward decl.
	class CTreeItem;
// private:
// 	class CTreeNode; //nested class
public:
//	CTreeNode *m_pRoot;//one and only
	// Overrides
...
public:
	int GetNextCilentNumber(int iteraNum);
	//creates a root
