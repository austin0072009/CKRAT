...
void CScreenSpyDlgSmall::OnClose()
{
...
void CScreenSpyDlgSmall::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CScreenSpyDlgSmall)
		// NOTE: the ClassWizard will add DDX and DDV calls here
	//}}AFX_DATA_MAP
}

BEGIN_MESSAGE_MAP(CScreenSpyDlgSmall, CDialog)
	//{{AFX_MSG_MAP(CScreenSpyDlgSmall)
...
/////////////////////////////////////////////////////////////////////////////
// CScreenSpyDlgSmall message handlers

void CScreenSpyDlgSmall::OnReceiveComplete()
{
...
bool CScreenSpyDlgSmall::SaveSnapshot()
{
...
DWORD WINAPI CScreenSpyDlgSmall::ShowFPSThread(LPVOID lpParam)
{
	CScreenSpyDlgSmall *pThis = (CScreenSpyDlgSmall *)lpParam;
...
void CScreenSpyDlgSmall::OnReceive()
{
	if (m_pContext == NULL)
		return;
...
void CScreenSpyDlgSmall::SendResetScreen(int	nBitCount)
{
	BYTE	bBuff[2];
...
void CScreenSpyDlgSmall::SendResetAlgorithm(UINT nAlgorithm)
{
	BYTE	bBuff[2];
...
BOOL CScreenSpyDlgSmall::OnInitDialog() 
{
...
void CScreenSpyDlgSmall::FullScreen()
{	
...
void CScreenSpyDlgSmall::ResetScreen()
{
	UINT	nBitmapInfoSize = m_pContext->m_DeCompressionBuffer.GetBufferLen() - 1;
...
void CScreenSpyDlgSmall::DrawFirstScreen()
{
...
void CScreenSpyDlgSmall::DrawNextScreenHome()
{
...
bool CScreenSpyDlgSmall::JPG_BMP(int cbit, void *input, int inlen, void *output)
{
...
void CScreenSpyDlgSmall::DrawNextScreenXvid()
{
	// 根据鼠标是否移动和屏幕是否变化判断是否重绘鼠标, 防止鼠标闪烁
	bool	bIsReDraw = false;
	int		nHeadLength = 1 + 1 + sizeof(POINT) + sizeof(BYTE); // 标识[1] + 算法[1] + 光标位置[8] + 光标类型索引[1]
...
void CScreenSpyDlgSmall::OnPaint() 
{
...
LRESULT CScreenSpyDlgSmall::OnSizing(WPARAM wParam, LPARAM lParam)
{
...
void CScreenSpyDlgSmall::OnSize(UINT nType, int cx, int cy) 
{
	...
	//	GetScrollBarInfo(OBJID_HSCROLL, &sbit);
...
void CScreenSpyDlgSmall::OnNcLButtonDblClk(UINT nHitTest, CPoint point)
{
...
void CScreenSpyDlgSmall::OnSysCommand(UINT nID, LPARAM lParam)
{
	CMenu* pSysMenu = GetSystemMenu(FALSE);
	switch (nID)
	{
...
void CScreenSpyDlgSmall::OnGetMinMaxInfo(MINMAXINFO* lpMMI)
{
	RECT rectWindow, rectClient;
	GetWindowRect(&rectWindow);
	GetClientRect(&rectClient);
...
void CScreenSpyDlgSmall::DrawTipString(CString str)
{
	RECT rect;
	GetClientRect(&rect);
...
BOOL CScreenSpyDlgSmall::PreTranslateMessage(MSG* pMsg) 
{
...
void CScreenSpyDlgSmall::PostNcDestroy() 
{
	// TODO: Add your specialized code here and/or call the base class
...
void CScreenSpyDlgSmall::SendCommand(MSG* pMsg)
{
	if (!m_bIsCtrl)
		return;
...
void CScreenSpyDlgSmall::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	// TODO: Add your message handler code here and/or call default
	SCROLLINFO si;
	si.cbSize = sizeof(SCROLLINFO);
...
void CScreenSpyDlgSmall::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	// TODO: Add your message handler code here and/or call default
	SCROLLINFO si;
	si.cbSize = sizeof(SCROLLINFO);
...
void CScreenSpyDlgSmall::UpdateLocalClipboard(char *buf, int len)
{
...
void CScreenSpyDlgSmall::SendLocalClipboard()
{
	if (!::OpenClipboard(NULL))
		return;
...
void CScreenSpyDlgSmall::SendNext()
{
	BYTE	bBuff = COMMAND_NEXT;
	m_iocpServer->Send(m_pContext, &bBuff, 1);
}

LRESULT CScreenSpyDlgSmall::OnMoving(WPARAM wParam, LPARAM lParam)
{
...
void CScreenSpyDlgSmall::OnMove(int x, int y) 
{
	CDialog::OnMove(x, y);
...
LRESULT CScreenSpyDlgSmall::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
	// TODO: Add your specialized code here and/or call the base class
